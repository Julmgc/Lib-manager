...
export const verifyIfEmailExists = async (
  ...
  try {
    const withEmail = await UserServices.getByEmail(req.validatedFields.email);
    if (withEmail) {
      throw new ApiError("E-mail already exists", 409);
    }

    const withCpf = await UserServices.findByCpf(req.validatedFields.cpf);
    if (withCpf) {
      throw new ApiError("Cpf already exists", 409);
    }
